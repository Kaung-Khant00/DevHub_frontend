...
const CommentCard = ({ c, user, handleUpdateComment, deleteCommentApi }) => {
  const deleteLoading = useSelector(
    (state) => state.post.comment.deleteLoading
  );
  return (
    <article
      key={c.id}
      className="flex gap-4 p-3 rounded-lg border border-base-200 hover:shadow-sm transition-shadow"
    >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              >
                {c.user?.name}
                {c?.user?.id === user?.id && (
                  <span className="px-2 text-sm text-base-content/60">You</span>
                )}
              </a>
